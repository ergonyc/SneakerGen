...
class Sampler_Z(tfk.layers.Layer):
    ...
    def call(self, inputs):
        ...
        dim_z = tf.shape(mu)[1]
        batch_size,dim_z = tf.shape(mu)
        z_sample = mu + sd * tf.random.normal(shape=(batch_size, dim_z))
        return z_sample, sd


def Conv2D(filters=32,
                    kernel_size=KERNEL_SZ,strides=2,padding="SAME",
                    activation="relu",kernel_regularizer=regularizers.l2(REGULARIZE_FACT)
                    ):
...
def Conv2DTranspose(filters=32,
                    kernel_size=KERNEL_SZ,strides=2,padding="SAME",
                    activation="relu",kernel_regularizer=regularizers.l2(REGULARIZE_FACT)
                    ):
...
class Encoder_Z(tfk.layers.Layer):
    ...
    def __init__(self, dim_z=64, pix_dim=192, name="encoder", **kwargs):
        super(Encoder_Z, self).__init__(name=name, **kwargs)
        self.pix_dim = pix_dim
        self.dim_x = (self.pix_dim, self.pix_dim, 3)
        self.dim_z = dim_z
        ...
        self.conv_layer_0 = Conv2D(filters=16)
...
class Decoder_X(tfk.layers.Layer):
    ...
    def __init__(self, dim_z, name="decoder", **kwargs):
        ...
        self.dim_z = dim_z
        self.dense_z_input = tfkl.Dense(self.pix_dim*self.pix_dim*32*3, activation=None)
        self.reshape_layer = tfkl.Reshape((self.pix_dim,self.pix_dim,32,3))
        self.conv_transpose_layer_4 = Conv2DTranspose(filters=1)
