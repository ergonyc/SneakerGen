...
@st.cache
def loadUMAPData2D(axes="2d"):
    """[used to be loadTSNEData2D ... just a helper for getTSNE2dData]

    Args:
        axes (str, optional): [description]. Defaults to '2d'.

    Returns:
        [pd.DataFrame]: [dataframe of the pre-computed UMAP]
    """
    if axes is "2d":
        fname = os.path.join(os.getcwd(), "data/df_sl_2d.csv")
    else: #3d
        fname = os.path.join(os.getcwd(), "data/df_sl_3d.csv")
    return pd.read_csv(fname)


@st.cache
def getUMAP2DData():
    df_tsne = loadUMAPData2D()
...
@st.cache
def loadSnk2Vec():
    infile = open(os.path.join(os.getcwd(), "data/snk2vec.pkl"), "rb") 
    snk2vec = pickle.load(infile)
    infile.close()
    mids = list(snk2vec.keys())  # also filenames...
    vecs = np.array([shape2vec[m] for m in mids])
    return snk2vec, mids, vecs


@st.cache
def loadMid2Desc():
    infile = open(os.path.join(os.getcwd(), "data/mid2desc.pkl"), "rb")  #line 570 (very end)
...
def getImg(text, snkmodel, textmodel, nlp):
    ptv = padEnc(text, nlp)
